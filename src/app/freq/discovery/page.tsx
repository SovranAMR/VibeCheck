...
import { FREQUENCY_QUESTIONS } from '@/lib/questions';
...
export default function FrequencyDiscoveryPage() {
  ...
  useEffect(() => {
    ...
    const playedFrequencies = session.fixedFreq.map(item => item.f);
    setMainTestFrequencies(playedFrequencies);
  }, [isUnlocked, router, setCurrentStep, session.fixedFreq]);
  ...
  const handleNext = () => {
    if (currentData.valence && currentData.locus.length > 0) {
      ...
      setFixedFreqData(fixedFreqData);
